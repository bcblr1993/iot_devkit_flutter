...
* **Hard Requirement**: All user-visible text (including tooltips and log messages) **MUST NOT be hardcoded** (neither English nor Chinese).
* **Process Specification**: Before implementing any UI changes or notification messages, you must first define the corresponding multi-language keys in the `.arb` files under `lib/l10n/`.
* **Code Reference**: All text must be retrieved using `AppLocalizations (l10n)` to ensure real-time switching between Chinese and English.
* **Specific Steps**:
