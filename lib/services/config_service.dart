...
class ConfigService {
  ...
  static Future<({bool success, String? error, bool cancelled})> exportToFile(
  ...
  static Future<({Map<String, dynamic>? config, String? error})>
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (signature != _appSignature) {
          return (
            config: null,
            error: '配置文件版本不兼容：签名为 "$signature"，期望 "$_appSignature"。'
          );
        }
  ...
  static Map<String, dynamic> _serializeConfig(Map<String, dynamic> config) {
    ...
    if (result['custom_keys'] != null &&
        result['custom_keys'] is List<CustomKeyConfig>) {
      result['custom_keys'] = (result['custom_keys'] as List<CustomKeyConfig>)
          .map((e) => e.toJson())
          .toList();
    }

    if (result['groups'] != null && result['groups'] is List<GroupConfig>) {
      result['groups'] = (result['groups'] as List<GroupConfig>)
          .map((e) => e.toJson())
          .toList();
    }
