...
class _TimestampToolState extends State<TimestampTool> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          if (!_timezones.any((t) => t['value'] == _selectedTsTz)) {
            _selectedTsTz = 'Asia/Shanghai';
          }
          if (!_timezones.any((t) => t['value'] == _selectedDtTz)) {
            _selectedDtTz = 'Asia/Shanghai';
          }
        });
      }
    } catch (e) {
  ...
  void _convertDateToTs() {
    ...
    try {
      DateTime parsed =
          DateTime.parse(dateStr.replaceAll('/', '-').replaceAll(' ', 'T'));

      final tzInfo = _timezones.firstWhere((e) => e['value'] == _selectedDtTz);
      final offsetHours = tzInfo['offset'] as int;

      final wallTimeAsUtc = DateTime.utc(parsed.year, parsed.month, parsed.day,
      ...
      try {
        ...
        final tzInfo =
            _timezones.firstWhere((e) => e['value'] == _selectedDtTz);
        final offsetHours = tzInfo['offset'] as int;
        final wallTimeAsUtc = DateTime.utc(parsed.year, parsed.month,
            parsed.day, parsed.hour, parsed.minute, parsed.second);
        final realUtc = wallTimeAsUtc.subtract(Duration(hours: offsetHours));
        setState(() {
          _conversionResultTs = realUtc.millisecondsSinceEpoch.toString();
          _setStatus(l10n.formatSuccess, Colors.green);
                ...
                children: [
                  ...
                  SelectableText(
                    ...
                    style: TextStyle(
                        fontSize: 28,
                        fontWeight: FontWeight.bold,
                        color: isPlaceholder || resultValue == 'Error'
