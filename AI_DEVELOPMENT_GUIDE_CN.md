...
* **强制要求**：所有用户界面可见的文本（甚至是工具提示和日志消息）**严禁使用硬编码字符串**（不管是中文还是英文）。
* **流程规范**：在实现任何 UI 变更或提示信息前，必须先在 `lib/l10n/` 下的 `.arb` 文件中定义对应的多语言 Key。
* **代码引用**：代码中必须统一使用 `AppLocalizations (l10n)` 进行文本获取，确保支持中英文实时切换。
* **具体步骤**：
